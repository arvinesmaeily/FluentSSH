...
public sealed partial class ConfigurationsPage : Page
{
    ...
    private void ConfigItem_Loaded(object sender, RoutedEventArgs e)
    {
        if (sender is Border border && border.DataContext is SSHConfiguration config)
        {
            var activeId = Main.ConnectionState.ActiveConfiguration?.Id ?? -1;

            var indicator = FindChildByName<Border>(border, "SelectionIndicator");
            if (indicator != null)
            {
                indicator.Visibility = config.Id == activeId ? Visibility.Visible : Visibility.Collapsed;
            }
        }
    }
